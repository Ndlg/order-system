...
10. 新建图片与档口资料页面骨架。
11. 新建匹配复核、异常处理、导出中心页面骨架。
12. 沿用当前 token 和 X-Workspace-Id 请求头。
13. 管理后台继续接入现有可用资源 API。
14. 未完成后端能力使用空状态和占位入口，未写死业务字段。
15. 前端 build 已通过。
16. 后端基础测试已通过。
17. 本地浏览器已验证 /workbench、/admin 和旧资源地址重定向。
```
