...
系统识别面单模式
  ↓
系统按平台共用解析模板生成客户可读面单信息
  ↓
用户在 Web 界面定义如何基于面单信息做关键字段、商品、图片、档口匹配和导出
  ↓
...
2026-06-11 修正：第一版数据隔离模型升级为“单平台数据库 + 多租户逻辑隔离 + 多工作区”。`tenant_id` 表示客户/租户归属，`workspace_id` 表示客户工作区归属和当前访问控制边界。所有客户业务数据必须同时保存 `tenant_id` 和 `workspace_id`；服务端后台只维护客户、工作区、用户和平台共用的面单解析模式/模板，不维护采集、字段、图片、档口、异常和导出等客户业务数据。

2026-06-11 追加校正：面单解析模式/模板不是客户工作区配置，而是平台共用能力。采集器读到的原始内容可能是杂乱 JSON / 文本，平台负责用统一解析规则整理成客户可读面单信息；客户只维护如何基于这些面单信息匹配商品、图片、档口、汇总和导出。

2026-06-11 追加校正：同一个本地打印组件不能简单等同于同一种面单解析模式。必须按来源平台、原始负载形态和可读字段能力拆分平台解析模板。例如菜鸟组件里，店铺直接打印能提供结构化订单字段；通过 woda 等打印平台中转时，标准面单字段可能被加密，但只要用户自定义区 / printXML 中能读取到商品、备注、规格等可见文字，就应进入后续匹配和导出主流程。

2026-06-11 追加校正：业务主线关注的是“本轮读取到的商品信息文字”，物流单号、平台订单号、店铺名等只是辅助字段。`standard_details` 应优先承载可用于商品、图片、档口匹配的文字内容；只有完全没有读到可用商品文字的采集内容，才进入未识别/诊断区。

2026-06-11 追加校正：商品与 SKU 的关系必须是“客户先定义商品，再把该商品对应的 SKU 图片 ZIP 导入到商品下面”。商品名称不由 ZIP 或文件名推断；ZIP 里的图片只生成当前商品下的 SKU 和 SKU 图片绑定。商品 / SKU 页面不维护匹配关键词；关键词映射商品 / SKU 必须放到专门匹配页面。后续匹配应从面单商品文字匹配到商品，再匹配商品下 SKU，最后得到图片和档口。

2026-06-11 追加校正：平台解析层已经提供标准字段候选，例如商品信息、规格文本、数量、来源、辅助单号等。客户管理页不应再让客户“定义候选字段”，也不应让客户先理解“字段用途 / 关键字段”这类抽象配置。客户侧应直接进入“商品识别”：使用服务端解析出的商品文字和规格文字，配置关键词如何关联商品 / SKU。
...
一个 MySQL 数据库
多个 Workspace 工作空间
平台维护共用面单解析模式和模板；每个 Workspace 维护自己的原始采集记录、标准化明细、字段用途、关键字段匹配、图片、档口、报货批次和导出记录
```
...
Workspace
  ↓
Raw Captures / Standard Details / Field Definitions / Key Fields / Match Rules / Images / Stalls / Reports / Exports

Platform Global
  ↓
Waybill Modes / Waybill Templates / Template Fields
```

要求：

```text
1. tenant_id 是客户/租户归属字段，workspace_id 是当前访问控制和工作区隔离字段。
...
order-system/
  backend/                    # FastAPI 后端
  frontend/                   # Vue 3 前端，租户端 UI 与平台后台 UI 独立入口
...
src/views/login
src/views/client
src/views/client-admin
src/views/server-admin
src/router/clientRoutes.ts
src/router/clientAdminRoutes.ts
src/router/serverAdminRoutes.ts
src/router/index.ts                 # 租户端 UI 路由，不包含平台后台
src/router/serverAdminIndex.ts      # 平台后台 UI 路由
```
...
管理页面面向客户公司管理员，维护当前 workspace 的采集连接、字段定义、关键字段、图片、档口、匹配规则和导出配置。

平台后台 UI 面向平台开发者 / 服务端管理员，只维护客户 / 租户账户、工作空间映射、用户、平台共用面单解析模式和面单解析模板。

平台后台 UI 必须使用独立入口和独立端口，不能混入租户端 UI 构建产物，也不能暴露给租户。

采集器连接、采集任务、原始采集记录、字段定义、图片、档口、异常和导出数据都属于租户工作空间数据，必须放在业务页面或管理页面中处理。
...
用户在客户业务 UI 点击开始采集
  ↓
服务端向当前 workspace 下的业务机采集器下发本轮采集任务
  ↓
用户正常使用菜鸟 / 抖店等打印组件打印订单
  ↓
用户在客户业务 UI 点击结束采集
  ↓
采集器关闭本轮采集并回传原始打印 / 面单信息
  ↓
服务端保存 raw_capture_records
  ↓
Web 端可查看、下载原始采集数据
```
...
员工账号只负责在业务 UI 发起开始采集和结束采集。
采集任务记录 started_by / ended_by 或等价审计字段。
...
管理员：管理所属 workspace 的用户、字段用途、匹配配置和业务数据；不维护平台共用面单解析模板。
...
面单模板用于把原始采集内容转换为客户可读面单信息。该模板属于平台共用解析逻辑，不属于单个客户 workspace。
...
capture_batches
raw_capture_records
waybill_modes          # 平台共用解析模式
waybill_templates      # 平台共用解析模板
...
第一版必须保证平台后台 UI 与租户端 UI 端口分离。P0 业务闭环、采集连接和采集数据必须落到租户端 UI，不进入平台后台 UI。
...
```text
上传一份包含面单信息的 Excel 或采集原始内容后，系统先按平台共用解析规则生成客户可读面单信息，再根据当前 workspace 的字段用途、关键字段匹配规则、图片和档口配置，生成报货 Excel。
```
...
2. 后端 OpenAPI 可访问。
3. 前端登录后可进入管理后台。
4. 可创建工作空间、平台面单解析模式、用户字段用途、关键字段组合、图片信息和档口。
...
15. 保留平台后台 UI，但必须使用独立私有端口验证平台控制面资源。
16. 搭建租户端业务页面和管理页面，并按面单批次处理流程串起 P0 闭环。
17. 补齐 Windows 脚本和 README。
18. 编写基础测试与示例数据。
```
