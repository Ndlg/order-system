...
系统按面单模板生成标准化明细
  ↓
用户在 Web 界面定义字段角色、映射关系、图片关系、档口关系和导出方式
  ↓
...
8. 不同用户维护不同规则、鞋款定义、规格映射、图片绑定、档口归属和导出配置。
...
一个 MySQL 数据库
多个 Workspace 工作空间
每个 Workspace 维护自己的面单模式、模板、标准化明细、字段角色、映射、图片、档口、报货批次和导出记录
```
...
Waybill Mode
  ↓
Templates / Standard Details / Field Roles / Mappings / Images / Stalls / Reports / Exports
```
...
完整原文：从面单或打印信息中提取的完整商品 / 订单原文。
店铺名：普通字段，可能为空，不能作为系统一级对象。
商品简称：普通字段，后续由用户指定是否作为业务简称 / 鞋款简称。
规格：普通字段，后续由用户定义规格映射。
尺码：普通字段，后续由用户定义尺码展示方式。
数量：普通字段，默认允许为 1。
备注：普通字段。
图片识别：展示字段，可用于提示图片是否已绑定。
任务ID、文档ID、任务时间、采集端ID、来源机器、来源序号：采集追踪字段。
原始内容：原始采集内容，不限定 JSON。
```

注意：字段名称是第一版标准化明细默认列，不代表系统业务规则写死。用户仍必须通过字段角色配置来定义每个字段的用途。
...
图片必须按 workspace 隔离，重新上传或批量导入关系。
...
```text
按 标准分类 / 标准鞋款 + 标准规格 + 商品图片 + 档口 分组
尺码合并展示
数量汇总
备注保留
```

注意：默认行为只能作为配置默认值，不能写成不可变业务规则。
...
2. 鞋款档口 Sheet
   一个 Excel 文件，按用户配置的分类 / 档口拆成多个 Sheet。

3. 鞋款档口文档
   按用户配置的分类 / 档口拆成多个 Excel 文件，打包 ZIP 下载。
```
...
```text
上传一份标准化 Excel 后，系统可以根据当前 workspace 的面单模式和用户维护的配置，生成报货 Excel。
```
...
2. 后端 OpenAPI 可访问。
3. 前端登录后可进入管理后台。
4. 可创建工作空间、面单模式、档口、业务简称 / 鞋款简称映射、规格映射。
5. 可上传商品图片并绑定到标准分类 / 标准鞋款 + 标准规格。
6. 可上传标准化 Excel。
7. 可生成标准化明细批次。
8. 可自动识别未映射简称、未映射规格、图片缺失、档口缺失异常。
9. 修正异常后可重新生成报货结果。
...
7. 实现标准化 Excel 上传与解析。
8. 实现字段角色配置。
9. 实现业务简称 / 鞋款简称映射、规格映射、档口管理、图片管理。
10. 实现报货汇总引擎。
11. 实现异常处理。
12. 实现 Excel 三模式导出。
13. 实现原始采集记录和面单模板框架。
